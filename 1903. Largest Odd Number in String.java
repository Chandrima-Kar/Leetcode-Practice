...
class Solution {
    public String largestOddNumber(String num) {
        for (int i = num.length() - 1; i >= 0; i--)// traversing from end to find largest odd number
        {
            char a = num.charAt(i);// last character
            int b = Character.getNumericValue(a);// taking numeric value
            if (b % 2 == 1) {
                return num.substring(0, i + 1);// if the last digit is odd return the number till that digit
            }
        }
        return ""; // retun empty string
    }
}
